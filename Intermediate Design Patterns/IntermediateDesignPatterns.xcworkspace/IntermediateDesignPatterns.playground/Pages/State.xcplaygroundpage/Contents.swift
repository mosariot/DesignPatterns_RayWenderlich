...
public class TrafficLight: UIView {
    ...
    public func transition(to state: TrafficLightState) {
        ...
        nextState.apply(to: self, afterDelay: currentState.delay)
    }
...
extension TrafficLightState {
    ...
    public func apply(to context: TrafficLight, afterDelay: TimeInterval) {
...
extension SolidTrafficLightState {
    ...
    public static func greenLight(color: UIColor = UIColor(red: 0.21, green: 0.78, blue: 0.35, alpha: 1),
                                 canisterIndex: Int = 2,
                                 delay: TimeInterval = 1.0) -> SolidTrafficLightState {
        SolidTrafficLightState(canisterIndex: canisterIndex, color: color, delay: delay)
    }
    
    public static func yellowLight(color: UIColor = UIColor(red: 0.98, green: 0.91, blue: 0.07, alpha: 1),
                                 canisterIndex: Int = 1,
                                 delay: TimeInterval = 0.5) -> SolidTrafficLightState {
        SolidTrafficLightState(canisterIndex: canisterIndex, color: color, delay: delay)
    }
    
    public static func redLight(color: UIColor = UIColor(red: 0.88, green: 0.0, blue: 0.04, alpha: 1),
                                 canisterIndex: Int = 0,
